...
When('I create an account', function() {
  ...
  cy.get('#form-validate').within(function() {
    ...
    cy.get('input[name="email"]').type('testuser@wishlist.example.com')
...
Then('A wishlist {string} event should be sent to Drip', function(type) {
  ...
  cy.wrap(Mockclient.retrieveRecordedRequests({
    ...
    expect(event.email).to.eq('testuser@wishlist.example.com')
...
When('I remove the {string} widget from my wishlist via {string}', function(type, method) {
  ...
  switch(method) {
    case 'quantity':
      cy.get('input[class="input-text qty validate-not-negative-number"]').type('{selectall}0')
